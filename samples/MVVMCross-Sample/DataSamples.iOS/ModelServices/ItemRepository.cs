...
namespace DataSamples.ModelServices
{
    ...
    public class ItemRepository : IItemRepository
    {
        ...
        public IEnumerable<Item> GetAll()
        {
            if (_items == null)
            {
                XDocument doc = XDocument.Load(typeof(ItemRepository).Assembly.GetManifestResourceStream("DataSamples.Core.Assets.Data.Inventories.xml"));
        ...
        private Item CreateItem(XElement x)
        {
            ...
            {
                item._thumbnailImage = typeof(ItemRepository).Assembly.GetManifestResourceStream("DataSamples.Core.Assets.Images.Thumbnail." + item.Image).ToByte();
